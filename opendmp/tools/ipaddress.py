...
def get_best_data_conn(peer_addresses):
    '''return a socket connected to the given address, possibly 
    with best connection time'''
    # TODO: better selection of best peer address
    Best_IP = []
    for peer in peer_addresses:
        ...
        try:
            ...
            if (result == 0): # Connection test ran fine
                Best_IP.append((s, t.interval))
    
    if (len(Best_IP) > 0):
                Best_IP.sort(key=lambda speed: speed[1])
                # close slower sockets
                for n in range(1,len(Best_IP)):
                    try:
                        Best_IP[n][0].close()
                    except (TypeError, socket.error, socket.gaierror) as e:
                        stdlog.error('Cannot close connection: ' + repr(e))
                return Best_IP[0][0]
    else:
        stdlog.error('Cannot connect to ' + ip_addr)
    
def get_data_conn(peer_addresses):
    '''return a socket connected to the given address'''
    for peer in peer_addresses:
        ip_addr = str(ip_address(peer.ip_addr))
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
